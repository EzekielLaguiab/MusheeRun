...
(function () {
  ...
  function sfxDoubleJump() {
    ...
    [[380, 0.07], [520, 0.07], [700, 0.1]].forEach(([freq, dur], i) => {
      const osc = ctx2.createOscillator();
      const g   = ctx2.createGain();
      osc.type = "sine";
  ...
  function sfxCollectStar() {
    playTone([
      [784, 0.06], [988, 0.06], [1175, 0.06], [1568, 0.14],
    ], "triangle", 0.2);
  }
  ...
  function sfxPowerup() {
    playTone([
      [392, 0.05], [494, 0.05], [587, 0.05], [698, 0.05], [880, 0.15],
    ], "square", 0.15);
  }
  ...
  function sfxEnemyHit() {
    ...
    osc.start(); osc.stop(ctx2.currentTime + 0.25);
  }

  // ---- Sound: Game Over ----
  function sfxGameOver() {
    playTone([
      [523, 0.15], [494, 0.15], [440, 0.15], [392, 0.3],
    ], "sawtooth", 0.2);
  }
  ...
  function sfxLevelUp() {
    playTone([
      [523, 0.08], [659, 0.08], [784, 0.08], [1047, 0.08], [1319, 0.18],
    ], "triangle", 0.22);
    // Extra sparkle layer
    const ctx2 = getAudioCtx();
    const t = ctx2.currentTime + 0.1;
    [[1568, 0.06], [1976, 0.12]].forEach(([freq, dur], i) => {
      const osc = ctx2.createOscillator();
      const g   = ctx2.createGain();
      osc.type = "sine";
  ...
  const BG_MELODY = [
    ...
    [880, 0.15], [784, 0.15], [698, 0.15], [659, 0.30],
    [523, 0.15], [494, 0.15], [523, 0.15], [587, 0.15],
    [659, 0.15], [587, 0.15], [523, 0.15], [494, 0.30],
    [440, 0.15], [494, 0.15], [523, 0.15], [587, 0.15],
    [659, 0.15], [784, 0.15], [880, 0.15], [1047,0.30],
    [880, 0.15], [784, 0.15], [659, 0.15], [587, 0.15],
    [523, 0.15], [494, 0.15], [440, 0.15], [392, 0.30],
  ];

  const BG_BASS = [
    [131, 0.30], [147, 0.30], [165, 0.30], [175, 0.30],
    [131, 0.30], [147, 0.30], [165, 0.30], [175, 0.30],
    [110, 0.30], [123, 0.30], [131, 0.30], [147, 0.30],
    [98,  0.30], [110, 0.30], [123, 0.30], [131, 0.30],
  ];
  ...
  function _killBgOscs() {
    ...
    bgActiveOscs = [];
    if (bgGainNode) {
      try { bgGainNode.gain.cancelScheduledValues(0); } catch(e) {}
      try { bgGainNode.gain.setValueAtTime(0, 0); } catch(e) {}
  ...
  function stopBgMusic() {
    ...
    if (bgSchedulerTimer) { clearTimeout(bgSchedulerTimer); bgSchedulerTimer = null; }
    _killBgOscs();
  }
  ...
  function scheduleBgMusic(gen) {
    ...
    for (const [freq, dur] of BG_MELODY) {
      ...
      osc.onended = () => { const idx = bgActiveOscs.indexOf(osc); if (idx > -1) bgActiveOscs.splice(idx, 1); };
      t += dur;
    }
  ...
  function update() {
    ...
    const diffConfig = DIFFICULTY_CONFIG[currentDifficulty];
    const obstacleInterval = Math.max(80 - level * 5 + diffConfig.obstacleIntervalMod, 30);
    if (frameCount - lastObstacleFrame > obstacleInterval + Math.random() * 40) {
      spawnObstacle();
      lastObstacleFrame = frameCount;
    }

    // Spawn collectibles (scales to level 10 + difficulty)
    const collectInterval = Math.max(60 - level * 3 + diffConfig.collectIntervalMod, 25);
    ...
    for (let i = obstacles.length - 1; i >= 0; i--) {
      ...
      if (o.flying) {
        ...
        // Ground enemies bob
        o.y = GROUND - o.h + Math.sin(frameCount * 0.08 + o.phase) * 6;
      }
      ...
      if (!invincible && hitCooldown === 0 && collidesAABB(player, o)) {
        takeDamage();
        spawnHitParticles(player.x + player.w / 2, player.y + player.h / 2, "#e74c3c");
        obstacles.splice(i, 1);
      }
    }
  ...
  function spawnObstacle() {
    ...
    const types = [
      // Ground obstacles
      { emoji: "🍄", color: "#8B0000", w: 52, h: 52, speed: 0.5, enemy: false, label: "poison", flying: false },
      { emoji: "🪨", color: "#777",    w: 58, h: 48, speed: 0.8, enemy: false, label: "rock", flying: false },
      { emoji: "🌵", color: "#2e7d32", w: 42, h: 72, speed: 0.3, enemy: false, label: "cactus", flying: false },
      { emoji: "🐛", color: "#b71c1c", w: 68, h: 52, speed: 1.5, enemy: true,  label: "bug", flying: false },
      { emoji: "🐌", color: "#e65100", w: 66, h: 50, speed: 0.6, enemy: true,  label: "slug", flying: false },
      // Flying obstacles - SMALLER size
      { emoji: "🦅", color: "#8B4513", w: 42, h: 42, speed: 1.2, enemy: true,  label: "eagle", flying: true },
      { emoji: "🦇", color: "#6a1b9a", w: 40, h: 40, speed: 1.4, enemy: true,  label: "bat", flying: true },
      { emoji: "🐝", color: "#ffd700", w: 38, h: 38, speed: 1.0, enemy: true,  label: "bee", flying: true },
      { emoji: "🦋", color: "#ff1744", w: 36, h: 36, speed: 0.8, enemy: true,  label: "butterfly", flying: true },
    ];
  ...
  function makeCloud(x, initial) {
    ...
    const speedMult = layer === 0 ? 0.08 + Math.random() * 0.06 : 0.15 + Math.random() * 0.1;
  ...
  function drawCloud(c, theme) {
    ...
    for (let i = 0; i < puffs; i++) {
      ...
      const centreness = 1 - Math.abs((i - (puffs - 1) / 2) / ((puffs + 1) / 2));
      const pr = (h * 0.5 + centreness * h * 0.35) * (0.75 + Math.sin(phase + i) * 0.12);
      ctx.fillStyle = cc.base;
      ctx.beginPath();
  ...
  function drawBgTree(t, theme) {
    ...
    switch (variety) {
      case 0: drawRoundTree(x, baseY, size, theme); break;
      case 1: drawPineTree(x, baseY, size, theme); break;
      case 2: drawFatBush(x, baseY, size, theme); break;
      case 3: drawPalmTree(x, baseY, size, theme); break;
      case 4: drawDoublePuff(x, baseY, size, theme); break;
    }
  ...
  function drawRoundTree(x, baseY, size, theme) {
    ...
    ctx.beginPath(); ctx.arc(x, canopyY, canopyR, 0, Math.PI * 2); ctx.fill();
    ctx.globalAlpha *= 0.7;
    ctx.beginPath(); ctx.arc(x - size*0.18, canopyY + size*0.08, canopyR*0.7, 0, Math.PI*2); ctx.fill();
    ctx.beginPath(); ctx.arc(x + size*0.18, canopyY + size*0.1, canopyR*0.65, 0, Math.PI*2); ctx.fill();
  }
  ...
  function drawFatBush(x, baseY, size, theme) {
    ...
    ctx.beginPath(); ctx.ellipse(x, bushY, size*0.5, size*0.3, 0, 0, Math.PI*2); ctx.fill();
    ctx.globalAlpha *= 0.8;
    ctx.beginPath(); ctx.ellipse(x - size*0.25, bushY + size*0.05, size*0.35, size*0.22, 0, 0, Math.PI*2); ctx.fill();
    ctx.beginPath(); ctx.ellipse(x + size*0.25, bushY + size*0.05, size*0.32, size*0.22, 0, 0, Math.PI*2); ctx.fill();
  }
  ...
  function drawPalmTree(x, baseY, size, theme) {
    ...
    ctx.quadraticCurveTo(x + size*0.15*lean, baseY - size*0.5, x + size*lean*0.3, baseY - size*0.85);
    ctx.quadraticCurveTo(x + size*0.1*lean + 4, baseY - size*0.5, x + 4, baseY);
    ctx.closePath();
    ctx.fill();
    // Fronds
  ...
  function drawDoublePuff(x, baseY, size, theme) {
    ...
    // Left puff
    ctx.beginPath(); ctx.arc(x - size*0.22, topY, size*0.3, 0, Math.PI*2); ctx.fill();
    // Right puff
    ctx.beginPath(); ctx.arc(x + size*0.22, topY - size*0.04, size*0.3, 0, Math.PI*2); ctx.fill();
    // Center overlap
    ctx.beginPath(); ctx.arc(x, topY - size*0.06, size*0.28, 0, Math.PI*2); ctx.fill();
  }
  ...
  function drawPlayer(theme) {
    ...
    const capColors  = ["#e53935","#7B1FA2","#1565C0","#e91e63","#e65100","#0277bd","#b71c1c","#006064","#6a1b9a","#37474f"];
    const capColors2 = ["#b71c1c","#4a148c","#0d47a1","#ad1457","#bf360c","#01579b","#7f0000","#004d40","#4a148c","#212121"];
    const rimColors  = ["#ef9a9a","#ce93d8","#90caf9","#f48fb1","#ffcc80","#b3e5fc","#ff8a80","#80deea","#e1bee7","#b0bec5"];
    ...
    const gloss = ctx.createRadialGradient(-R*0.28, -R*0.1, 0, -R*0.2, -R*0.05, R*0.42);
    ...
    ctx.quadraticCurveTo(-R * 0.75, capY + R * 0.36, -R * 0.92, capY + R * 0.12);
    ctx.closePath();
    ctx.fill();

    // Cap dome — main colour
    const capGrad = ctx.createRadialGradient(-R*0.18, capY - R*0.22, R*0.04, 0, capY, R*0.75);
    ...
    const capGloss = ctx.createLinearGradient(-R*0.3, capY - R*0.9, -R*0.05, capY - R*0.2);
    ...
    ctx.quadraticCurveTo(-R*0.08, capY - R*0.25, -R*0.12, capY - R*0.72);
    ctx.closePath();
    ctx.fill();
    ...
    const spots = [
      ...
      { x: -R*0.38,  y: capY - R*0.28, r: R*0.10 },
      { x:  R*0.38,  y: capY - R*0.28, r: R*0.10 },
    ...
    for (const sp of spots) {
      ctx.beginPath();
      ctx.arc(sp.x - sp.r*0.25, sp.y - sp.r*0.25, sp.r * 0.5, 0, Math.PI * 2);
      ctx.fill();
    }
    ...
    ctx.fillStyle = "#fff";
    ctx.beginPath(); ctx.ellipse(-eyeX, faceY, R*0.22, isBlinking ? R*0.04 : R*0.26, 0, 0, Math.PI*2); ctx.fill();
    ctx.beginPath(); ctx.ellipse( eyeX, faceY, R*0.22, isBlinking ? R*0.04 : R*0.26, 0, 0, Math.PI*2); ctx.fill();
    ...
    if (!isBlinking) {
      ...
      ctx.beginPath(); ctx.ellipse(-eyeX + pupilShift*0.4, faceY + 2, R*0.13, R*0.17, 0, 0, Math.PI*2); ctx.fill();
      ctx.beginPath(); ctx.ellipse( eyeX + pupilShift*0.4, faceY + 2, R*0.13, R*0.17, 0, 0, Math.PI*2); ctx.fill();

      // Eye shine — two dots (DFO style)
      ctx.fillStyle = "#fff";
      ctx.beginPath(); ctx.arc(-eyeX - R*0.05, faceY - R*0.08, R*0.065, 0, Math.PI*2); ctx.fill();
      ctx.beginPath(); ctx.arc(-eyeX + R*0.06, faceY - R*0.03, R*0.035, 0, Math.PI*2); ctx.fill();
      ctx.beginPath(); ctx.arc( eyeX - R*0.05, faceY - R*0.08, R*0.065, 0, Math.PI*2); ctx.fill();
      ctx.beginPath(); ctx.arc( eyeX + R*0.06, faceY - R*0.03, R*0.035, 0, Math.PI*2); ctx.fill();
    }
    ...
    ctx.beginPath(); ctx.ellipse(-eyeX - R*0.08, faceY + R*0.22, R*0.2, R*0.1, -0.3, 0, Math.PI*2); ctx.fill();
    ctx.beginPath(); ctx.ellipse( eyeX + R*0.08, faceY + R*0.22, R*0.2, R*0.1,  0.3, 0, Math.PI*2); ctx.fill();
    ...
    // ==============================
    const armSwing = player.onGround ? Math.sin(t * 0.35) * 22 : (player.vy < 0 ? -35 : 20);
    ...
    ctx.beginPath(); ctx.arc(R*0.04, R*0.42, R*0.1, 0, Math.PI*2); ctx.fill();
    ctx.restore();
  ...
  function drawObstacles() {
    for (const o of obstacles) {
      ...
      if (o.flying) {
        ...
        const cx = o.x + o.w / 2;
        const cy = o.y + o.h / 2;
        const wobble = Math.sin(frameCount * 0.18 + o.phase) * 0.2;
        ctx.translate(cx, cy);
        ctx.rotate(wobble);

        if (o.label === "bug") {
          drawEnemyBug(ctx, 0, 0, o.w, o.h, frameCount);
        } else {
          drawEnemySlug(ctx, 0, 0, o.w, o.h, frameCount);
        }
  ...
  function drawPoisonMushroom(ctx, x, y, w, h) {
    ...
    // Dark spots on stalk
    ctx.fillStyle = "#bbb";
    ctx.beginPath(); ctx.arc(cx - 4, y + h * 0.65, 3, 0, Math.PI * 2); ctx.fill();
    ctx.beginPath(); ctx.arc(cx + 5, y + h * 0.75, 2.5, 0, Math.PI * 2); ctx.fill();
    ...
    const spots = [{ x: 0, y: -h*0.2, r: 6 }, { x: -w*0.2, y: -h*0.1, r: 4.5 }, { x: w*0.2, y: -h*0.1, r: 4.5 }];
    for (const sp of spots) {
      ctx.beginPath();
    ...
    // Angry eyes
    ctx.fillStyle = "#fff";
    ctx.beginPath(); ctx.arc(cx - w * 0.14, y + h * 0.56, 5, 0, Math.PI * 2); ctx.fill();
    ctx.beginPath(); ctx.arc(cx + w * 0.14, y + h * 0.56, 5, 0, Math.PI * 2); ctx.fill();
    ctx.fillStyle = "#c00";
    ctx.beginPath(); ctx.arc(cx - w * 0.14, y + h * 0.57, 3, 0, Math.PI * 2); ctx.fill();
    ctx.beginPath(); ctx.arc(cx + w * 0.14, y + h * 0.57, 3, 0, Math.PI * 2); ctx.fill();
    // Angry brows
    ctx.strokeStyle = "#4a0072";
    ctx.lineWidth = 2.5;
    ctx.lineCap = "round";
    ctx.beginPath(); ctx.moveTo(cx - w*0.22, y + h*0.51); ctx.lineTo(cx - w*0.06, y + h*0.54); ctx.stroke();
    ctx.beginPath(); ctx.moveTo(cx + w*0.06, y + h*0.54); ctx.lineTo(cx + w*0.22, y + h*0.51); ctx.stroke();
    // Label
    ctx.fillStyle = "#fff";
  ...
  function drawRock(ctx, x, y, w, h) {
    ...
    const cy = y + h / 2;
    // Rock body
    const rg = ctx.createRadialGradient(cx - w*0.1, cy - h*0.1, 2, cx, cy, w * 0.6);
    ...
    ctx.beginPath();
    ctx.ellipse(cx - w*0.12, cy - h*0.15, w*0.22, h*0.15, -0.5, 0, Math.PI*2);
    ctx.fill();
    // Crack lines
    ctx.strokeStyle = "rgba(0,0,0,0.3)";
    ctx.lineWidth = 1.5;
    ctx.beginPath(); ctx.moveTo(cx, y + h*0.2); ctx.lineTo(cx - w*0.1, y + h*0.5); ctx.stroke();
    ctx.beginPath(); ctx.moveTo(cx + w*0.1, y + h*0.3); ctx.lineTo(cx + w*0.25, y + h*0.6); ctx.stroke();
  }
  ...
  function drawCactus(ctx, x, y, w, h) {
    ...
    ctx.lineWidth = 1.5;
    const spinePos = [
      [cx - w*0.28, y + h*0.45], [cx - w*0.28, y + h*0.6], [cx - w*0.28, y + h*0.75],
      [cx + w*0.28, y + h*0.45], [cx + w*0.28, y + h*0.6], [cx + w*0.28, y + h*0.75],
    ];
    ...
    ctx.beginPath(); ctx.arc(cx - 5, y + h * 0.5, 3, 0, Math.PI*2); ctx.fill();
    ctx.beginPath(); ctx.arc(cx + 5, y + h * 0.5, 3, 0, Math.PI*2); ctx.fill();
    ctx.strokeStyle = "#1B5E20";
    ctx.lineWidth = 2;
    ctx.beginPath(); ctx.arc(cx, y + h * 0.58, 5, 0.1, Math.PI-0.1); ctx.stroke();
  }
  ...
  function drawEnemyBug(ctx, cx, cy, w, h, frame) {
    const t = frame * 0.15;
    // Body
    const bg = ctx.createRadialGradient(cx - w*0.1, cy - h*0.1, 1, cx, cy, w*0.5);
    ...
    ctx.beginPath(); ctx.arc(cx - w*0.15, cy - h*0.1, w*0.09, 0, Math.PI*2); ctx.fill();
    ctx.beginPath(); ctx.arc(cx + w*0.15, cy - h*0.1, w*0.09, 0, Math.PI*2); ctx.fill();
    ctx.beginPath(); ctx.arc(cx - w*0.12, cy + h*0.1, w*0.07, 0, Math.PI*2); ctx.fill();
    ctx.beginPath(); ctx.arc(cx + w*0.12, cy + h*0.1, w*0.07, 0, Math.PI*2); ctx.fill();
    // Head
    ...
    ctx.beginPath(); ctx.arc(cx - w*0.12, cy - h*0.45, 5, 0, Math.PI*2); ctx.fill();
    ctx.beginPath(); ctx.arc(cx + w*0.12, cy - h*0.45, 5, 0, Math.PI*2); ctx.fill();
    ctx.fillStyle = "#fff";
    ctx.beginPath(); ctx.arc(cx - w*0.1, cy - h*0.47, 2, 0, Math.PI*2); ctx.fill();
    ctx.beginPath(); ctx.arc(cx + w*0.14, cy - h*0.47, 2, 0, Math.PI*2); ctx.fill();
    // Antennae
    ctx.strokeStyle = "#212121";
    ctx.lineWidth = 2;
    ctx.beginPath(); ctx.moveTo(cx - w*0.1, cy - h*0.55); ctx.lineTo(cx - w*0.22, cy - h*0.78); ctx.stroke();
    ctx.beginPath(); ctx.moveTo(cx + w*0.1, cy - h*0.55); ctx.lineTo(cx + w*0.22, cy - h*0.78); ctx.stroke();
    ctx.fillStyle = "#ff1744";
    ctx.beginPath(); ctx.arc(cx - w*0.22, cy - h*0.78, 3, 0, Math.PI*2); ctx.fill();
    ctx.beginPath(); ctx.arc(cx + w*0.22, cy - h*0.78, 3, 0, Math.PI*2); ctx.fill();
    // Animated legs
    ctx.strokeStyle = "#212121";
    ctx.lineWidth = 2;
    for (let i = 0; i < 3; i++) {
      ...
      ctx.save(); ctx.translate(lx, ly); ctx.rotate(-legSwing);
      ctx.beginPath(); ctx.moveTo(0, 0); ctx.lineTo(-w*0.28, h*0.12); ctx.stroke();
      ctx.restore();
      ctx.save(); ctx.translate(cx + w * 0.42, ly); ctx.rotate(legSwing);
      ctx.beginPath(); ctx.moveTo(0, 0); ctx.lineTo(w*0.28, h*0.12); ctx.stroke();
      ctx.restore();
    }
    // Warning glow
  ...
  function drawEnemySlug(ctx, cx, cy, w, h, frame) {
    ...
    ctx.beginPath();
    ctx.ellipse(cx + w*0.1, cy + h*0.3, w*0.6, h*0.18, 0, 0, Math.PI*2);
    ctx.fill();
    // Body
    ...
    ctx.fillStyle = sg;
    ctx.beginPath();
    ctx.ellipse(cx + bodyWave, cy + h*0.05, w * 0.48, h * 0.32, 0, 0, Math.PI * 2);
    ctx.fill();
    // Belly
    ctx.fillStyle = "rgba(255,220,150,0.7)";
    ctx.beginPath();
    ctx.ellipse(cx + bodyWave, cy + h*0.15, w*0.36, h*0.18, 0, 0, Math.PI*2);
    ctx.fill();
    // Shell on back
    ctx.fillStyle = "#8D6E63";
    ctx.beginPath();
    ctx.ellipse(cx + bodyWave - w*0.05, cy - h*0.12, w*0.28, h*0.28, -0.3, 0, Math.PI*2);
    ctx.fill();
    // Shell spiral
    ctx.strokeStyle = "#5D4037";
    ctx.lineWidth = 1.5;
    ctx.beginPath();
    ctx.arc(cx + bodyWave - w*0.05, cy - h*0.12, w*0.15, 0, Math.PI*1.5);
    ctx.stroke();
    ctx.beginPath();
    ctx.arc(cx + bodyWave - w*0.05, cy - h*0.12, w*0.08, 0.2, Math.PI*1.3);
    ctx.stroke();
    // Head
    ctx.fillStyle = "#FF8F00";
    ctx.beginPath();
    ctx.ellipse(cx - w * 0.38 + bodyWave, cy - h*0.02, w*0.22, h*0.24, -0.2, 0, Math.PI*2);
    ctx.fill();
    // Eyes on stalks
    ...
    // Eye balls
    ctx.fillStyle = "#fff";
    ctx.beginPath(); ctx.arc(cx - w*0.48 + bodyWave, cy - h*0.4 + eyeWave, 5, 0, Math.PI*2); ctx.fill();
    ctx.beginPath(); ctx.arc(cx - w*0.3 + bodyWave, cy - h*0.42 + eyeWave, 5, 0, Math.PI*2); ctx.fill();
    ctx.fillStyle = "#212121";
    ctx.beginPath(); ctx.arc(cx - w*0.47 + bodyWave, cy - h*0.41 + eyeWave, 2.5, 0, Math.PI*2); ctx.fill();
    ctx.beginPath(); ctx.arc(cx - w*0.29 + bodyWave, cy - h*0.43 + eyeWave, 2.5, 0, Math.PI*2); ctx.fill();
    // Smile (smug)
